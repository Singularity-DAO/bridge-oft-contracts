...
- adapter parameters: allows UAs to add arbitrary transaction params in the send() function, 
  like airdrop on destination chain. unlike a real LayerZero endpoint, it is
...
contract LZEndpointMock is ILayerZeroEndpoint {
    ...
    event PayloadStored(uint16 srcChainId, 
                        bytes srcAddress, 
                        address dstAddress, 
                        uint64 nonce, 
                        bytes payload, 
    ...
    function send(uint16 _chainId, 
                  ...
                  bytes memory _adapterParams) external payable override sendNonReentrant {
        ...
        (uint nativeFee, ) = estimateFees(_chainId, 
                                          msg.sender, 
                                          _payload, 
    ...
    function receivePayload(uint16 _srcChainId, 
        ...
        if (sp.payloadHash != bytes32(0)) {
            ...
            storedPayload[_srcChainId][_path] = StoredPayload(uint64(_payload.length), 
                                                              _dstAddress, 
                                                              keccak256(_payload));
            ...
            try ILayerZeroReceiver(_dstAddress).lzReceive{gas: _gasLimit}(_srcChainId, 
                                                                          _path, 
                                                                          _nonce, 
                                                                          _payload) {} catch (bytes memory reason) {
                storedPayload[_srcChainId][_path] = StoredPayload(uint64(_payload.length), 
                                                                  _dstAddress, 
                                                                  keccak256(_payload));
    ...
    function retryPayload(uint16 _srcChainId, bytes calldata _path, bytes calldata _payload) external override {
        ...
        require(_payload.length == sp.payloadLength 
                && keccak256(_payload) == sp.payloadHash, "LayerZeroMock: invalid payload");
    ...
    function setRelayerPrice(uint128 _dstPriceRatio, 
                             uint128 _dstGasPriceInWei, 
                             uint128 _dstNativeAmtCap, 
                             uint64 _baseGas, 
                             uint64 _gasPerByte) external {
